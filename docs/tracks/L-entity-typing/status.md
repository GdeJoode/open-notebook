...
| L.2-fix | restore canonical-passthrough in _resolve_entity_type | — | `track/l2-canonical-passthrough-fix` | 2026-06-23 | adversarial-reviewer APPROVED (0 blockers/majors). Regression found in live validation: bare canonical labels (Organization/Person/Event) the LLM emits fell to `concept` instead of their type (L.2's resolve_residual_type dropped the old canonical-passthrough). Restored in order: L.1 bridge → passthrough → L.2 alias → fallback. Overlap check clean (only `technology`, self-mapped). 115 tests. |
