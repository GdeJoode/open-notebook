...
| Q.3 | Status assignment + migration 59 + change-log | NET-NEW logic, reuse status/properties | ready for review 🔍 | `track/q3-status` | migration 59 (manual_override bool COLUMN + status_change_log TABLE); pure StatusAssignmentService (tier×degree×override truth table, manual_override ALWAYS wins); EntityRepository.set_status (override-conditioned no-op); 30 tests green (24 unit + 6 testcontainer); migrations-roundtrip + entity-repo + relation-merge regressions green |
