...
| K.6 | Entity-resolution review hub + alias/overlay management + external-id badges | (pending) | `track/k6-resolution-ui` | 2026-06-22 | implemented — all 7 ACs green. Hub renders all states (loading skeleton / inline error+retry / empty 'No duplicates pending' / default). Approve gates on a destructive-merge confirm dialog showing the survivor + merged-away losers before `POST /apply` (AC2); reject records a reversible force-split overlay so the pair is never re-proposed (AC3); candidate list paginated (10/page). OverlayEditor reuses the J.5 Radix RadioGroup scope-toggle (Tab/arrow/Space, aria-label per option — AC5). ExternalIdBadges render TOOI/DOI as links, nothing when empty (AC6). KG page links to the hub + shows badges/alias-count on entity detail; EntitiesTab surfaces alias/external-id per entity; KG client + extraction types carry resolution fields. Tests: 15 unit (MergeClusterCard + OverlayEditor logic, node-env vitest) + 6 E2E (frontend/e2e/track-k). Full vitest 53/53 green, tsc clean, eslint clean on changed files. AliasManager wired to the K.5 overlay endpoints (no alias-CRUD endpoint exists — see escalations K.6). Ready for review |
| K.6 rev2 | review-fix: gate fuzzy-candidate merge behind the shared confirm dialog; defer AC4 (remove dead AliasManager) | (pending) | `track/k6-resolution-ui` | 2026-06-22 | adversarial review fixed (2 majors + 2 minors). **MAJOR1**: fuzzy-candidate "Merge" no longer bypasses the destructive gate — extracted a shared `MergeConfirmDialog` reused by both the cluster card and a new `CandidateMergeCard`; the candidate Merge now opens the same AlertDialog (survivor + loser by surface form) and only confirm issues `POST /apply`. New E2E asserts the candidate merge requires the dialog + no apply fires until confirmed. **MAJOR2 / AC4 DEFERRED**: deleted dead `AliasManager.tsx` (zero importers, never mounted, overlay-as-alias semantics misleading); per-entity alias add/remove needs a backend `POST/DELETE /entities/{id}/aliases` that does not exist → AC4 deferred to a K.6-followup (see escalations). Read-only alias count + ExternalIdBadges + OverlayEditor stay. **Minors**: confirm `AlertDialogAction` disabled while `applyMerge.isPending` (double-submit guard); `providerLabel` labels a bare `10.xxxx` as 'DOI' not 'ID'. Tests: vitest 64/64 green (added CandidateMergeCard + external-ids unit suites), tsc clean, eslint clean on changed files, 7 E2E parse (incl. new candidate-confirm), `create_app()` imports. Ready for review |
| K.7a | Type-aware relation endpoint resolution (persist + merge) | (pending) | `track/k7a-typesafe-relations` | 2026-06-22 | implemented — all 5 ACs green; additive + name-only fallback (zero regression). **AC1 (headline, @requires_docker)**: persist person-BZK + org-BZK + a relation from each → each edge anchors at the TYPE-CORRECT entity; a sibling falsifiability test runs the legacy name-only `LIMIT 1` and proves it can't tell the two apart (real guard). Verified the headline test FAILS when the persist resolution is reverted to name-only. **AC2**: cross-batch / unknown-type endpoint → name-only fallback still resolves (roundtrip green). **AC3 (B.8 intact)**: `hash_id`/`(canonical_name, entity_type)` ENTITY dedup UNCHANGED — K.7a touches only relation endpoint resolution; the 3 B.8 suites green (26 tests; the two roundtrip files byte-unmodified, persistence-service additive only). **AC4 (notebook_merge)**: `name_to_canon` re-keyed `(name, type)` consistent with the merge buckets; `_list_relations_for_notebook` projects endpoint `entity_type`; new test seeds a person-X / org-X sharing a normalized name → each relation routes to its own type's winner (fails under the old name-only key, proven). **AC5 (audit)**: `EntityRepository.audit_ambiguous_relation_endpoints` — read-only, counts endpoints whose canonical_name maps to >1 active typed entity (~0 under the conservative normalizer), logged, never mutates (read-only roundtrip asserts entity count + statuses unchanged). K.3 `repoint_relations` confirmed already ID-based → no change. `ExtractedRelation` gained `source_type`/`target_type: Optional[str] = None`. Tests: 3 shared model, 13 persistence unit (10 orig + 3 K.7a), 9 notebook_merge unit (8 orig + 1 K.7a), 4 endpoint-resolution roundtrip (incl. headline + audit). Validate: B.8 gate 26 green, shared `-k extraction or relation` 11 green, `create_app()` imports, ruff clean. Do NOT re-enable K.1 aggressive prefixes (that is K.7b, gated on this). Ready for review |
...
| K.7a | type-aware relation endpoint resolution | — | `track/k7a-typesafe-relations` | 2026-06-22 | adversarial-reviewer APPROVED (0 blockers/majors). Additive (name,type) endpoint resolution + name-only fallback; B.8 contract structurally untouched (roundtrip suites byte-identical, 26 green); homograph test proven to fail under old key; notebook_merge name_to_canon re-keyed (name,type); read-only audit. 42 tests. **K.7b minor**: same-batch type map is last-wins on a true cross-type homograph — once K.7b re-enables aggressive prefixes, prefer relation-carried source_type (the audit is the canary). |
